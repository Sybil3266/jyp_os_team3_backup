...
BOOL kFormat( void )
{
    ...
    // MBR 이후부터 루트 디렉터리 데이터 영역(0번 클러스터)까지 모두 0으로 초기화
    // 1번 클러스터도 0으로 초기화
    kMemSet( gs_vbTempBuffer, 0, 512 );
    for( i = 0 ; i < ( dwClusterLinkSectorCount + FILESYSTEM_SECTORSPERCLUSTER*2 -1 );
         i++ )
    {
        ...
        else if ( i < dwClusterLinkSectorCount + FILESYSTEM_SECTORSPERCLUSTER - 1)
        {
...
FILE* kOpenFile( const char* pcFileName, const char* pcMode )
{
    ...
    pstFile->stFileHandle.wUserPermission = stEntry.wUserPermission;
...
BOOL kUserPermissionCheck( const FILE* file, WORD yourPermission ){
    FILEHANDLE* pstFileHandle;
    pstFileHandle = &( file->stFileHandle );
    if( (pstFileHandle->wUserPermission & yourPermission) > 0 ){
      return 1;
    }
    else{
      return 0;
...
void kSetPermission(const char* pcFileName, WORD permissionVal){

    FILEHANDLE* pstFileHandle;
    DIRECTORYENTRY* pstDirectoryEntry;
    int iIndex;
    //pstFileHandle = &( file->stFileHandle );
    iIndex = kFindDirectoryEntry( pcFileName, pstDirectoryEntry);

    //kPrintf("\n%d %x %x\n", hel, permissionVal, pstDirectoryEntry->wUserPermission);
    pstDirectoryEntry->wUserPermission = permissionVal;
    kSetDirectoryEntryData( iIndex, pstDirectoryEntry );
    //pstFileHandle->wUserPermission = permissionVal;

    //kPrintf("\n%s %x %x\n", pstDirectoryEntry->vcFileName, permissionVal, pstDirectoryEntry->wUserPermission);
...
BOOL kInitUserInformation(USERINFORMATION* userInfo){
    DWORD dwCluster;
    // 빈 클러스터를 찾아서 할당된 것으로 설정
    dwCluster = kFindFreeCluster();
    if( ( dwCluster == FILESYSTEM_LASTCLUSTER ) ||
    ...
    USERINFORMATION* pstUserInfo;
    kReadCluster(1, gs_vbTempBuffer); //1번 클러스터 read
    pstUserInfo = (USERINFORMATION *)gs_vbTempBuffer;
    kMemCpy(pstUserInfo, userInfo, sizeof(USERINFORMATION));
    pstUserInfo->userCounter = (WORD)1;
    kWriteCluster(1, gs_vbTempBuffer);
    //gs_pfWriteHDDSector( TRUE, TRUE, userCount + gs_stFileSystemManager.dwClusterLinkAreaStartAddress, 1, userInfo);
    return TRUE;
}
//유저 정보를 1클러스터에 씀
BOOL kWriteUserInformation(DWORD index, USERINFORMATION* userInfo){
  USERINFORMATION* pstUserInfo;
  kReadCluster(1, gs_vbTempBuffer); //1번 클러스터 read

  pstUserInfo = (USERINFORMATION *)gs_vbTempBuffer;
  kMemCpy(pstUserInfo+index, userInfo, sizeof(USERINFORMATION));
...
BOOL kReadUserInformation(DWORD index, USERINFORMATION* userInfo){
  USERINFORMATION* pstUserInfo;
  kReadCluster(1, gs_vbTempBuffer);
  pstUserInfo = (USERINFORMATION *)gs_vbTempBuffer;
  kMemCpy(userInfo, pstUserInfo+index, sizeof(USERINFORMATION));
  if(index == 0 && kMemCmp(userInfo->UserID,"root",sizeof(MAXUSERIDLENGTH))==0){
      return FALSE;
  }
  //gs_pfReadHDDSector( TRUE, TRUE, index + gs_stFileSystemManager.dwClusterLinkAreaStartAddress, 1, userInfo);
  return TRUE;
}

BOOL kUpdateUserCount(DWORD count){
  USERINFORMATION* pstUserInfo;
  kReadCluster(1, gs_vbTempBuffer); //1번 클러스터 read

  pstUserInfo = (USERINFORMATION *)gs_vbTempBuffer;
  pstUserInfo->userCounter = count;
